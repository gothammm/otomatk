...
>>>>>>> parent of 3824b0c... Revert "Added extra condition in save command."
      }
  ...
  _generateSenecaActions(model) {
    ...
    this.seneca.add({ role: roleName, plugin: name, cmd: 'save' }, (args, done) => {
      ...
      if (!modelData._id) {
        modelData = R.pickBy((val, key) => R.keys(Model.schema).indexOf(key) >= 0, modelData);
        Model.create(modelData).then((data) => {
...
<<<<<<< HEAD
      ...
      if (modelData._id && !Model.hooks.onSaving) {
        modelData = R.pickBy((val, key) => R.keys(Model.schema).indexOf(key) >= 0, modelData);
...
=======
      } else if (modelData._id && Model.hooks.onSaving && !R.isArrayLike(modelData)) {
>>>>>>> parent of 3824b0c... Revert "Added extra condition in save command."
